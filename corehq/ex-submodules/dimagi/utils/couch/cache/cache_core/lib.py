...
# Separate caches for python versions.  Only permit one key at a time.
# TODO - this should work if cache is volatile.  But, if key->values are expected to persist, this will not work well
class RedisCacheByPythonVersion(object):
    ...
    def set_many(self, *args, **kwargs):
        ...
        self._other_cache.delete_many(*args, **kwargs)  # TODO - verify.  Might not need if never used in HQ
        return return_val
